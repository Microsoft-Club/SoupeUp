...
/// Extract a user-facing error from a background process.
/// Dask logs INFO to stderr by default — never surface raw stderr as the error message.
pub fn background_process_error(proc: &BackgroundProcessInfo) -> Option<String> {
    // Explicit error marker from our embedded scripts (stdout).
    for line in proc.stdout_tail.lines().rev() {
        if let Some(json) = line.strip_prefix("DASK_WORKER_ERROR ") {
            return Some(json.to_string());
        }
        if let Some(json) = line.strip_prefix("DASK_SCHEDULER_ERROR ") {
            return Some(json.to_string());
        }
    }

    // Real failures on stderr (tracebacks / ERROR level).
